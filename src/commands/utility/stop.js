 ...
 *
 * /stop - make the bot leave the voice channel.
...
module.exports = {
  data: new SlashCommandBuilder().setName('stop').setDescription('Stop the bot and leave the voice channel'),
  async execute(interaction) {
    ...
    try {
      await me.voice.disconnect('Stopped by user');
      destroyConnection(guild);
      return reply(interaction, { embeds: [successEmbed({ description: 'Stopped — I left the voice channel.' })] });
    } catch (err) {
